...
public class CallCenterFinderImpl extends CallCenterFinderBaseImpl implements CallCenterFinder{
	
	private static final Log _logger = LogFactoryUtil.getLog(CallCenterFinderImpl.class);
	
	public static final String LIST_ALL_CAMPAIGNS = CallCenterFinder.class.getName() + ".listAllCampaigns";
	
	public static final String LIST_ALL_CALLCENTER = CallCenterFinder.class.getName() + ".listAllCallCenter";
	...
	public List<CallCenter> getAllCallCenter() {
		...
		try {
			...
			String sql = CustomSQLUtil.get( getClass(), LIST_ALL_CALLCENTER);
			
			SQLQuery q = session.createSQLQuery(sql);
			q.setCacheable(false);
			q.addEntity("CallCenter", CallCenterImpl.class);
			
			return (List<CallCenter>) q.list(true);
		}
		catch (Exception e) {
			e.printStackTrace();
			try {
	...
	@Override
	public List findCallCenterReportEntryByPK(long callCenterScoringId) {
		// TODO Auto-generated method stub
		return null;
	}
